...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{uml/classSystem.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{uml/erPartialSystem.png}
...
According to MySQL documentation and guides, MySQL Workbench is a graphical tool for working with MySQL servers and databases. MySQL Workbench fully supports MySQL server versions 5.6 and higher. It is also compatible with older MySQL server 5.x versions, except in certain situations (like displaying the process list) due to changed system tables. It does not support MySQL server versions 4.x \cite{mclaughlin_2013_mysql_workbench}.
...
websites and web applications. It contains HTML- and CSS-based design templates for typography, forms, buttons, navigation and other interface components, as well as optional JavaScript extensions \cite{chahal_2019_what}.

\subsection{jQuery library}
jQuery is not a language, but it is a well written JavaScript code. As quoted on
official jQuery website, ``it is a fast and concise JavaScript Library that simplifies HTML document traversing, event handling, animating, and Ajax interactions for rapid web development'' \cite{chaffer_2007_learning}.
...
% TODO: Put proper appendix here
Following are few of the test cases designed for major system modules. Please refer Appendix E for all of the test cases and results.
...
After implementing the system, a user evaluation, also known as user acceptance testing was carried out. This was done in the client's enviroment with the participation of the client in order to ensure functionalites provided by the final system are cable of satisfying the operational needs of the company.

During this process the client was  introduced to the final system. But this introduction process was very brief because the client was already familiar with the system to some degree, due to that fact that his feedback was constantly taken while the development process was ongoing.
...
Then, other staff members of the business were asked to interact with the system. Therefore, staff members such as cashiers, shop managers, factory manager and factory supervisor used the system with their respective user accounts. This was observed by the owner to make sure privileges are working correctly. During this process, cashiers pointed out few small issues in the customer invoice module. Also, the factory supervisor requested couple of changes in production order confirm module.
...
Finally, the system was accepted by the client saying it will improve the efficiency of their manual process and help to carry out required tasks smoothly. The certificate received from the client has been appended to appendix G.

% TODO: update appendix here
...
\chapter*{\Huge Appendix A – System Documentation}
\addcontentsline{toc}{chapter}{\bf{Appendix A – System Manual}}
